...
#specials > h1,#specials > h2,#specials > h3,#specials > h4,#specials > h5,#specials > h6,#specials > p{color:#ffffff; font-weight:normal;margin:0px;} 
 ...
 #specials > .heading{padding-top:10px;font-size:40px;font-style: italic;}
...
#adboard > h1,#adboard > h2,#adboard > h3,#adboard > h4,#adboard > h5,#adboard > h6,#adboard > p{color:#ffffff; font-weight:normal;margin:0px;} 
 ...
 #adboarda {clear:left;margin-left:3%;width:90%;display:inline;float:left;  padding-left:40px;margin-top:500px;background:transparent url(../../images/caption-bg.png) 0 0 repeat;-moz-border-radius:20px;
-webkit-border-radius:20px;
-khtml-border-radius:20px;
border-radius:20px;
}
 #adboardb {width:49%;display:inline;float:right;  }
  
 #adboarda > .title{margin-top:25px;font-size:55px;font-weight:bold;}
 #adboarda > .heading{padding-top:10px;font-size:40px;font-style: italic;line-height:40px;}
 #adboarda > .subheading{padding-top:5px;font-size:40px;font-style: italic;line-height:40px;}
 #adboarda > .details{padding-top:5px;font-size:40px;font-style: italic;line-height:40px;}  
...
<!--<body style="background:transparent url(/images/randalls/logo_splash.jpg) 0 0 no-repeat;overflow:hidden;max-width:100%;">-->
<body style="background:#000000;overflow:hidden;max-width:100%;">
       <!-- JQM PAGE-->
       
       <div id="diag">
       <?php  
$con=mysqli_connect("localhost","datafog_beer","Df44556362","datafog_demo1");
// Check connection
if (mysqli_connect_errno()) {
  echo "Failed to connect to MySQL: " . mysqli_connect_error();
}

$result = mysqli_query($con,"SELECT * FROM ads WHERE atype = 'HTML'");

while($row = mysqli_fetch_array($result)) {
//echo '<code>';
//echo $row['ad_content'];
//echo '</code>';
preg_match_all('/<img[^>]+>/i',$row['ad_content'], $resultimgs); 
 //echo '<code>';
 //echo $resultimgs[0][0];
 //echo '</code>';
  preg_match_all('/(src)=("[^"]*")/i',$resultimgs[0][0], $img);
  // echo '<code>';
...
// echo '</code>';
  ...
  //echo '<pre>'.$row['ad_content'] . " " . $row['atype'].'</pre>';
  //echo "<br>";
...
mysqli_close($con);?>
    ...
    <div data-role="page" id="listbeergrid" data-theme="d" style="background: #000000;width:100%;" >
      <!--  <div data-role="header"  data-theme="b">
        ...
        <div data-role="content" style="padding:0px;" id="slides"> 
			...
			<div id="slideshow">	   
			 ...
			 <div id="vendorblock" style="position:absolute;top:0px;left:0px;z-index:10000;display:none;"><img alt="" src="../../images/datafog_logo_new.png" style="width:150px;margin-top:1840px;margin-left:20px;z-index:10000;" /> </div>
            </div>  
			<div id="logoblock" style="position:absolute;top:0px;left:0px;z-index:10000;"><!--<img alt="" src="../../images/datafog_logo_white.png" style="width:200px;margin-top:920px;margin-left:25px;z-index:10000;" />--><img alt="" src="/images/randalls/randalls_logo_white.png" style="width:250px;margin-top:1840px;margin-left:815px;z-index:10000;display:none;" /></div>
  ...
  <script>
	  			...
	  			//runBeerInfo();
	  		 	...
	  		 	// setTimeout('startVideo("kissing.mp4");', 20000);
	  		 	 ...
	  		 	 function startVideo(vidname,offset,coords) {
		  		 ...
		  		 $.ajax({
                    ...
                    cache: false,
                    //data: formData,
                     data: {vidname : vidname,offset : offset ,coords : coords},
       ...
       // $('#vendorblock').show(10);
		...
		$('#wines-gridview').html("");
     	$('#beers-gridview').html("");
            ...
            $('#beers-gridview').append('<a href="#" data-role="button" data-inline="true" data-theme="b" class="col1 company-block" style="min-height:200px;padding-top: 0px;padding-bottom: 0px;"><div id="test"><h1 style="font-size:40px;">Pour House Grill</h1><h1  style="font-size:50px;padding-top:20px;">Craft Beer Menu</h1></div></a></div><div id="beers-gridview2">'); 
...
// LOAD BEERS INTO GRID  (images from local stored files based on beer id            
           //$('#beers-gridview').append('<a id="'+data[key].id+'" href="#" data-role="button" data-inline="true" data-theme="b" class="col1"  style="min-width:1080px;"><img class="ui-li-thumb" src="../beerimages/'+data[key].id+'.jpg"/><h3 class="ui-li-heading beernamex">'+data[key].beer_name+'</h3><p class="beer-style beershadow">'+data[key].beer_style+'</p><p  class="brewery-name beershadow">'+data[key].brewery_name+'</p><p  class="brewery-city beershadow">'+data[key].brewery_city+'</p><p  class="ui-li-desc brewery-durl  beershadow">'+data[key].brewery_url+'</p><p  class="ui-li-desc beer-desc">'+data[key].beer_desc+'</p><p class="ui-li-desc beer-abv">ABV: '+data[key].beer_abv+'%</p><p class="ui-li-desc beer-ibu">IBUs: '+data[key].beer_ibu+'</p><p class="ui-li-desc beer-glass">Pint: $'+data[key].beer_glass_price+'</p><p class="ui-li-desc beer-growler">Growler: $'+data[key].beer_growler_price+'</p></a>');
           
            $('#beers-gridview2').append('<a id="'+data[key].id+'" href="#" data-role="button" data-inline="true" data-theme="b" class="col1"  style="min-width:1075px;background:#363636;"><img class="ui-li-thumb" src="../beerimages/'+data[key].id+'.jpg"/><h3 class="ui-li-heading beernamex">'+data[key].beer_name+'</h3><p class="beer-style beershadow">'+data[key].beer_style+'</p><p  class="brewery-name beershadow">'+data[key].brewery_name+'</p><p  class="brewery-city beershadow">'+data[key].brewery_city+'</p><p  class="ui-li-desc brewery-durl  beershadow">'+data[key].brewery_url+'</p><p  class="ui-li-desc beer-desc">'+data[key].beer_desc+'</p><p class="ui-li-desc beer-abv">ABV: '+data[key].beer_abv+'%</p><p class="ui-li-desc beer-ibu">IBUs: '+data[key].beer_ibu+'</p><p class="ui-li-desc beer-glass">Pint: $'+data[key].beer_glass_price+'</p><p class="ui-li-desc beer-growler">Growler: $'+data[key].beer_growler_price+'</p></a>');
...
// ADJUST BEER DESCRIPTION TO FIT                   
	...
	/*	 $('#beers-gridview').simplyScroll({
          ...
          //  setInterval(rotatebeers,2500);
           function rotatebeers() { 
           //var firstbeer = $('#beers-gridview a:eq(1)').fadeOut("slow");
           //$('#beers-gridview a:eq(1)').fadeOut("slow").remove();
          // $('#beers-gridview').append(firstbeer).trigger("create");
           //$('#beers-gridview a:eq(1)').remove();
           // $('#beers-gridview').trigger("refresh") 
           //var firstbeer=$('#beers-gridview a:eq(1)');
		   	
           var firstbeer = $('#beers-gridview a:eq(1)');
           var offset = $('#beers-gridview a:eq(1)').offset().top;
           $.mobile.silentScroll(offset);
          // $('#beers-gridview').append(firstbeer).trigger("create", function(){
         //$('#beers-gridview a:eq(1)').fadeOut(500, function() { $(this).remove(); });
   ...
   // });
         ...
         /*  setInterval(rotatebeers,1000);
           function rotatebeers() { 
           var lastbeer = $('#beers-gridview a:eq(22)');
           $('#beers-gridview a:eq(22)').remove();
           $('#beers-gridview a').css("border","none");
           $(lastbeer).insertAfter('#beers-gridview a:eq(0)').css("border","3px solid #cccccc").fadeIn("fast");
           //$('#beers-gridview').trigger('create').slideDown("slow");
           } */
// LOAD MARQUEE
...
// END LOAD MARQUEE       
	   ...
	   function listAd1(bgimg,adtitle,pos,set)
	   {
		...
		function onSuccessAd1(data, status)
        {
           ...
           $('#beers-gridview').html("");	
           // $('#specials-ad').html("");	          
           $('#specials-ad').html('<div id="adboard" class="dropshad" style="min-height:1920px;margin:0px;padding:0px;float:left;background: transparent url('+bgimg+') 0 0 no-repeat"><h2 class="header  class="textshadow"" style="text-align:center">'+adtitle+'</h2><div id="adboarda"></div><div id="adboardb"></div></div>');
			...
			spec2 ='<h2 class="title  textshadow">'+data.special_2+'</h2><h3 class="heading textshadow">'+data.special_2h+'</h3><h4 class="subheading textshadow">'+data.special_2s+'</h4><p  class="details textshadow">'+data.special_2d+'</p>';
			spec3 ='<h2 class="title  textshadow">'+data.special_3+'</h2><h3 class="heading textshadow">'+data.special_3h+'</h3><h4 class="subheading textshadow">'+data.special_3s+'</h4><p  class="details textshadow">'+data.special_3d+'</p>';
			spec4 ='<h2 class="title  textshadow">'+data.special_4+'</h2><h3 class="heading textshadow">'+data.special_4h+'</h3><h4 class="subheading textshadow">'+data.special_4s+'</h4><p  class="details textshadow">'+data.special_4d+'</p>';
			spec5 ='<h2 class="title  textshadow">'+data.special_5+'</h2><h3 class="heading textshadow">'+data.special_5h+'</h3><h4 class="subheading textshadow">'+data.special_5s+'</h4><p  class="details textshadow">'+data.special_5d+'</p>';
			spec6 ='<h2 class="title  textshadow">'+data.special_6+'</h2><h3 class="heading textshadow">'+data.special_6h+'</h3><h4 class="subheading textshadow">'+data.special_6s+'</h4><p  class="details textshadow">'+data.special_6d+'</p>';

			
			
			
			
			 $('#adboarda').append(spec1);
			 $('#adboarda').append(spec2);
			 $('#adboarda').append(spec3);
			 $('#adboarda').append(spec4);
			 $('#adboarda').append(spec5);
			 $('#adboarda').append(spec6);
   ...
   function listSpecials()
	   {
		...
		function onSuccessSpecials(data, status)
        {
           ...
           $('#beers-gridview').html("");	
           // $('#specials-ad').html("");
           	$('#specials-ad').css("background","transparent url(/images/front_counter.png) 0 0 no-repeat");          
           $('#specials-ad').html('<div id="specials" class="dropshad" style="display:inline-block;float:left;background: transparent url(/images/caption-bg.png) 0 0 repeat"><h2 class="header" style="text-align:center;padding-top:25px;">Today\'s Specials</h2></div><div style="display:inline-block;float:right;width:49%;margin-top:50px;"><h2 class="header" style="font-size:80px;font-style:italic;margin-top:0px;">Eat The Legend</h2><img src="/images/legend3.jpg" style="margin-top:80px;border:5px solid #000000;width:800px;"/><h2 class="header" style="font-size:80px;font-style:italic;margin-top:50px;text-align:center;">  Become One!</h2></div>');
           $('#specials-ad').append('<div id="slideshow-wrapper1"><div id="slideshow1"></div></div>');
           $('#slideshow1').append('<div><img src="/images/legend1.jpg"></div><div><img src="/images/legend2.jpg"></div><div><img src="/images/legend3.jpg"></div><div><img src="/images/legend4.jpg"></div><div><img src="/images/legend_leader_small.png"></div>');

			var spec1 ='<h2 class="title textshadow">'+data.special_1+'</h2><h3 class="heading">'+data.special_1h+'</h3><h4 class="subheading">'+data.special_1s+'</h4><p  class="details">'+data.special_1d+'</p>';
			$('#specials').append(spec1);
			var spec2 ='<h2 class="title textshadow">'+data.special_2+'</h2><h3 class="heading">'+data.special_2h+'</h3><h4 class="subheading">'+data.special_2s+'</h4><p  class="details" style="font-size:40px;">'+data.special_2d+'</p>';
			...
			var intid=setInterval(function() { 
				...
				cnts++;
				$('#testr').text(" ");
				if(cnts >= 12){clearInterval(intid);}
				},  4000);
